...
function showCommands(path) {
  say("Carregando os comandos das pastas", {
    font: "console",
    align: "left",
    gradient: ["red", "blue"],
  });
...
async function magneum() {
  ...
  logger.info("📢 Conectando à database Mongodb()...");
  try {
    await mogclient.connect(dbdata.MONGO_URL, {
      useNewUrlParser: true,
      useUnifiedTopology: true,
    });
    logger.info("📢 Conectado com o mongoose.");
  } catch (error) {
    logger.error("❌ Não é possível conectar-se ao Mongodb():", error);
    process.exit(0);
  }
  logger.info("📢 Conectando à database Sequelize()...");
  try {
    await sequelize.authenticate();
    logger.info("📢 A conexão foi estabelecida com sucesso.");
  } catch (error) {
    logger.error("❌ Não foi possível conectar-se ao Sequelize():", error);
    process.exit(0);
  }
  logger.info("📢 Sincronizando a database Sequelize()...");
