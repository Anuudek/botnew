...
async function magneum() {
  await monGoose
    ...
    .catch(async (error) => {
      logger.error("❌ Não foi possível conectar-se ao mangoose.");
      logger.error(error);
    })
    .then(logger.info("📢 Conectado ao mangoose."));
  const opage = express();
  ...
  opage.listen(PORT, logger.info("📢 botAnuudek se iniciou na porta " + PORT));
  ...
  try {
    ...
    logger.info(
      "📢 O state e saveCreds foram recuperados com sucesso de remote_authstate."
    );
  } catch (error) {
    logger.error("📢 Ocorreu um erro em remote_authstate:", error);
    logger.debug(
      "📢 Usando fallback_remote_authstate: recuperando estado e saveCreds de Reddis_RemoteFileAuthState."
    );
    ({ state, saveCreds } = await fallback_remote_authstate(logger));
    logger.info(
      "📢 O state e saveCreds foram recuperados com sucesso de fallback_remote_authstate."
    );
  }
  ...
  BloomBot.ev.on("connection.update", async (update) => {
    ...
    if (connection == "connecting") logger.info("📢 Conectando ao WhatsApp...");
    else if (connection == "open") logger.info("📢 Login bem-sucedido!");
    else if (connection == "close") {
      let reason = new Boom(lastDisconnect?.error)?.output.statusCode;
      if (reason === DisconnectReason.badSession) {
        logger.error(
          `❌ Arquivo de sessão inválido, exclua a sessão e faça o scan novamente`
        );
        BloomBot.logout();
        ...
        logger.error(`❌ Dispositivo desconectado, faça scan novamente e execute.`);
        process.exit(0);
      } else if (reason === DisconnectReason.restartRequired) {
        logger.debug("💡: É necessário reiniciar, reiniciando...");
        await magneum();
      } else if (reason === DisconnectReason.timedOut) {
        logger.error("❌ A conexão expirou, reconectando...");
        await magneum();
      } else
        BloomBot.end(
      ...
      logger.debug("💡: Notificações pendentes recebidas.");
    else if (receivedPendingNotifications === false)
      logger.error("📢 Notificações pendentes não recebidas.");
    else if (isNewLogin === true) logger.debug("💡: Novo login.");
    else if (isNewLogin === false) logger.error("📢 Não é novo login.");
    else if (qr) logger.info("Qr: "), console.log(qr);
    else logger.info("📢 Conexão...", update);
  });
...
> Eu sou o botAnuudekt.
> Para começar a usar digite .help ou pressione os botões abaixo.`,
  ...
  setInterval(async () => {
    const _Type = [
      ...
      "🧪Químico",
      "🗺️Cartógrafo",
      "🎢Designer de montanha-russa",
      "🎁Embalador de presente",
      "🚀Explorador espacial",
      "🔮Psíquico",
      "🌳Ecologista",
      "🎟️Cobrador",
      ...
      "🌋Vulcanologista",
      "🔐Chaveiro",
      "🎡Designer de parque de diversões",
      "🏰Arquiteto",
      "📓Jornalista",
